...
const ManageClass =({Class,index,refetch}) => {    
    ...
    const handleApprove = Class => {
        fetch(`https://pro-learn-server-lemon.vercel.app/class/approve/${Class._id}`, {
          method: 'PATCH'
        })
          .then(res => res.json())
          .then(data => {
    ...
    const handleDeny = Class => {
        fetch(`https://pro-learn-server-lemon.vercel.app/class/deny/${Class._id}`, {
          method: 'PATCH'
        })
          .then(res => res.json())
          .then(data => {
        ...
        if (status === 'denied') {
          Swal.fire({
            ...
            preConfirm: async (feedback) => {
              try {
                const response = await fetch(`https://pro-learn-server-lemon.vercel.app/class/feedback/${Class._id}`, {
                  method: 'PATCH',
                  headers: {
    ...
    //           }).then((result) => {
    //             if (result.isConfirmed) {
    //                 fetch(`https://pro-learn-server-lemon.vercel.app/class/feedback/${Class._id}`, {
    //                     method: 'PATCH'
    //                   })
    //             }
    //           })
